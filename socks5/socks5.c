...
static sock_t *sock_create(int fd, sock_state_t state, int isclient,
                           tunnel_t *tunnel) {
 ...
 * */
static void sock_force_shutdown(sock_t *sock) { sock_release(sock); }
...
static int sock_keepalive(int fd) {
    int keepalive = 1;
    return setsockopt(fd, SOL_SOCKET, SO_KEEPALIVE, &keepalive,
                      sizeof(keepalive));
}
...
static void tunnel_release(tunnel_t *tunnel) { free(tunnel); }
...
static void tunnel_read_handle(int fd, void *ud) {
    ...
    switch (tunnel->state) {
        ...
        case connected_state:
            if (tunnel_connected_handle(tunnel, sock->isclient) < 0)
...
methods:
    ...
    if (buff_readable(buff) >= op->nmethods) {
        ...
        reply[0] = 0x05;  // socks5
        int auth =
            strcmp(SERVER.username, "") != 0 && strcmp(SERVER.passwd, "");
        if (auth) {
            reply[1] = 0x02;
...
static int tunnel_notify_connected(tunnel_t *tunnel) {
    ...
    if (sa.sa_family == AF_INET) {
        ...
        if (tunnel_write_client(tunnel, &sa_in->sin_addr,
                                sizeof(sa_in->sin_addr)) < 0)
            return -1;
        if (tunnel_write_client(tunnel, &sa_in->sin_port,
                                sizeof(sa_in->sin_port)) < 0)
            return -1;
        ...
        tunnel_write_client(tunnel, &sa_in6->sin6_addr,
                            sizeof(sa_in6->sin6_addr));
        tunnel_write_client(tunnel, &sa_in6->sin6_port,
                            sizeof(sa_in6->sin6_port));
    } else {
...
static int tunnel_connect_to_remote(tunnel_t *tunnel) {
    ...
    if (getaddrinfo(addr, port, &ai_hint, &ai_list) != 0) {
        LOG("getaddrinfo failed,addr=%s,port=%s,error=%s", addr, port,
            gai_strerror(errno));
        return -1;
    }
    ...
    for (ai_ptr = ai_list; ai_ptr != NULL; ai_ptr = ai_ptr->ai_next) {
        ...
        if ((status = connect(newfd, ai_ptr->ai_addr, ai_ptr->ai_addrlen)) !=
                0 &&
            errno != EINPROGRESS) {
            close(newfd);
            newfd = -1;
...
static int tunnel_request_handle(tunnel_t *tunnel) {
    ...
    size_t *nreaded = &tunnel->read_count;
    size_t nheader =
        sizeof(rp->ver) + sizeof(rp->cmd) + sizeof(rp->rsv) + sizeof(rp->atyp);
...
static int tunnel_connecting_handle(tunnel_t *tunnel) {
    ...
    int code =
        getsockopt(tunnel->remote_sock->fd, SOL_SOCKET, SO_ERROR, &error, &len);
    /*
...
static int tunnel_connected_handle(tunnel_t *tunnel, int client) {
    if (client) {
        ...
        if (buff_concat(tunnel->remote_sock->write_buff,
                        tunnel->client_sock->read_buff) < 0)
            return -1;
        ...
        if (buff_concat(tunnel->client_sock->write_buff,
                        tunnel->remote_sock->read_buff) < 0)
            return -1;
...
static void accept_handle() {
    ...
    if ((newfd = accept(SERVER.listenfd, NULL, NULL)) < 0) {
        LOG("accept_handle failed,listenfd=%d,err=%s", SERVER.listenfd,
            strerror(errno));
        return;
    }
...
static int server_init(char *host, char *port, char *username, char *passwd) {
    ...
    int reuse = 1;
    setsockopt(listenfd, SOL_SOCKET, SO_REUSEADDR, (void *)&reuse,
               sizeof(reuse));
