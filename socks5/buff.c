...
buff_t *buff_create(size_t capacity) {
    buff_t *buff = (buff_t *)malloc(sizeof(buff_t) + capacity);
    if (buff == NULL) {
        return NULL;
    }
    buff->read_index = 0;
...
static size_t buff_prependable(buff_t *buff) { return buff->read_index; }
...
int buff_concat(buff_t *front, buff_t *rear) {
    return buff_write(front, rear->data + rear->read_index,
                      buff_readable(rear));
}
